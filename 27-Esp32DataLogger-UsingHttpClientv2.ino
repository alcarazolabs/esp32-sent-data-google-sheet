...
const char* serverName = "xxxxx"; //GOOGLE APPS SCRIPT URL
void setup() {
...
//subrutina
void sendToGoogleSheet(float temperature, int humidity){
  if(WiFi.status() == WL_CONNECTED){
    HTTPClient http;
    http.begin(serverName);
    http.addHeader("Content-Type", "application/json");
  
    String jsonData = "{\"temp\":\"" + String(temperature) + "\", \"hum\":\"" + String(humidity)+ "\"}";
